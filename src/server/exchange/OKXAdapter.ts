...
export class OKXAdapter implements IExecutionAdapter {
  ...
  /**
   * Formats quantity in contracts or units adhering to OKX lot step & min size
   */
  public async formatQuantity(symbol: string, desiredQty: number, currentPrice: number): Promise<number> {
    ...
    // If desiredQty is expressed in base tokens (e.g. 0.05 BTC), convert to contracts: desiredQty / ctVal
